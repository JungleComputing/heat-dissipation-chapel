...
proc do_compute() {
  var env_var : c_string;
  sys_getenv("CHPL_RT_NUM_THREADS_PER_LOCALE", env_var);
  writeln("Running with CHPL_RT_NUM_THREADS_PER_LOCALE: ",
	  env_var : string);
  ...
  for iteration in 1..I {
    ...
    max_difference = max reduce [ij in CylinderDomain]
      abs(dst.temperature[ij] - src.temperature[ij]);
    reductionTimer.stop();
